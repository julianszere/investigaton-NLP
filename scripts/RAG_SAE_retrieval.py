...
from src.agents.RAG_SAE import RAG_SAE
...
results_dir = (
    ...
    f"SAE_{sae_release.replace('/', '_')}_{sae_id}_embeddings_{config.embedding_model_name.replace('/', '_')}"
    f"_generator_{config.memory_model_name.replace('/', '_')}"
